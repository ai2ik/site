<?php
	...
	add_shortcode("view-live"			, "ewf_projects_sc_viewlive"); 
	...
	add_image_size( 'project-preview-medium', 280, 170, true);
	...
	function ewf_projects_sc_filter($atts, $content){
		...
		extract(shortcode_atts(array(
			"overviewurl" => get_option(EWF_SETUP_THNAME."_page_portfolio", null ),
			"overviewtitle" =>  __("All Work", EWF_SETUP_THEME_DOMAIN),
			"showoverview" => "true"
		), $atts)); 
		...
		if (is_array($terms)){
			$src.= '<ul id="portfolio-filter" class="fixed">';
			$src.= '<li><a href="'.$overviewurl.'">'.$overviewtitle.'</a>';
				$src.= '<ul class="filter-options">';
	...
	function ewf_projects_sc_slider($atts, $content){
		...
		if (count($arrImages) > 0){
			$src .= '<div id="slideshow-portfolio">';
					if($arrImages) {					
						$src .= '<ul>';
						...
						foreach($arrImages as $oImage) {
							if ($featuredimage == 'false'){
								if ($oImage->ID != $image_id){
									...
									$src .= '<li><img height="350" width="880" alt="" src="'.$cr_image_large_preview[0].'" style="opacity: 1;"></li>'; 
								}
							}else{
								$cr_image_large_preview = wp_get_attachment_image_src( $oImage->ID, 'project-preview-large');						
								$src .= '<li><img height="350" width="880" alt="" src="'.$cr_image_large_preview[0].'" style="opacity: 1;"></li>';  
						...
						$src .= '</ul><div id="slideshow-portfolio-pager">&nbsp;</div>';
					}
			$src .= '</div>';  
	...
	function ewf_projects_sc_overview($atts, $content){ 
		...
		extract(shortcode_atts(array(
			...
			"order" => "DESC",
			"hassidebar" => "false",
			"itemsperrow" =>  get_option(EWF_SETUP_THNAME."_portfolio_items_per_row","3") ,
			"service" => null,
		), $atts));
		...
		if ($hassidebar == 'true' && $itemsperrow == '3'){
		...
		$src.= '<div class="portfolio-overview"><ul class="fixed">';
		...
		while ($wp_query_project->have_posts()) : $wp_query_project->the_post();
			...
			$extra_class = null;
			if ($count_items_column == $itemsperrow) {  $extra_class = 'last';  }
			...
			$src .= '<li class="'.$extra_class.'">';
					...
					if ($image_id) {
						...
						$src .= '<div><img src="'.$image_large_preview[0].'" width="280" height="170" alt="" /></div>';  
					}
		...
		if ($nav == 'true' && $wp_query_project->max_num_pages > 1){
			$src .= ewf_projects_pagination(4, $wp_query_project, $navposition);
		}
